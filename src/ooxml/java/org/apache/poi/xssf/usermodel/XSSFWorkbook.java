...
public class XSSFWorkbook extends POIXMLDocument implements Workbook, Iterable<XSSFSheet> {
    ...
    /**
     * Excel silently truncates long sheet names to 31 chars. This constant is
     * used to ensure uniqueness in the first 31 chars
     */
    ...
    /**
     * A collection of shared objects used for styling content, e.g. fonts, cell
     * styles, colors, etc.
     */
    private StylesTable stylesSource;
    private ThemesTable theme;
    /**
     * The locator of user-defined functions. By default includes functions from
    ...
    /**
     * Used to keep track of the data formatter so that all createDataFormatter
     * calls return the same one for a given book. This ensures that updates
     * from one places is visible someplace else.
     */
    private XSSFDataFormat formatter;
    /**
     * The policy to apply in the event of missing or blank cells when fetching
     * from a row. See {@link org.apache.poi.ss.usermodel.Row.MissingCellPolicy}
     */
     ...
     * <code>Package</code> object, see <a
     * href="http://poi.apache.org/oxml4j/">http://poi.apache.org/oxml4j/</a>.
     *
     * Once you have finished working with the Workbook, you should close the
     * package by calling pkg.close, to avoid leaving file handles open.
     *
     ...
     * memory and then opening an {@link OPCPackage} object for it.
     *
     ...
     * the underlying .zip file stream. In short, there are two ways to open a
     * OPC package:
     * </p>
     * <ol>
     * <li>
     * from file which leads to invoking java.util.zip.ZipFile(File file) deep
     * in POI internals.
     * </li>
     * <li>
     ...
     * {@link #XSSFWorkbook(org.apache.poi.openxml4j.opc.OPCPackage)}
     * constructor:
     * <pre><code>
     *       OPCPackage pkg = OPCPackage.open(path);
     *       XSSFWorkbook wb = new XSSFWorkbook(pkg);
    ...
    /**
     * Generate a valid sheet name based on the existing one. Used when cloning
     * sheets.
     *
     ...
     *
     * <p>
     * Note that Excel allows sheet names up to 31 chars in length but other
     * applications (such as OpenOffice) allow more. Some versions of Excel
     * crash with names longer than 31 chars, others - truncate such names to 31
     * character.
     * </p>
     * <p>
     * POI's SpreadsheetAPI silently truncates the input argument to 31
     * characters. Example:
     *
     * <pre><code>
     ...
     * <p>
     * Sheet name MUST be unique in the workbook and MUST NOT contain the any of
     ...
     * <p>
     * See
     * {@link org.apache.poi.ss.util.WorkbookUtil#createSafeSheetName(String nameProposal)}
     * for a safe way to create valid names
     * </p>
     *
     * @param sheetname sheetname to set for the sheet.
     * @return Sheet representing the new sheet.
     * @throws IllegalArgumentException if the name is null or invalid or
     * workbook already contains a sheet with this name
     ...
     * <i>Note:</i>Excel named ranges are case-insensitive and this method
    ...
    /**
     * Retrieves the reference for the printarea of the specified sheet, the
     * sheet name is appended to the reference even if it was not specified.
     *
     ...
     * @return XSSFSheet with the name provided or <code>null</code> if it does
     * not exist
     */
    ...
    /**
     * As {@link #removeName(String)} is not necessarily unique (name + sheet
     * index is unique), this method is more accurate.
     *
     ...
     * selected sheet in the workbook. There are a few situations when Excel
     * must have a selection and/or active sheet. (For example when printing -
     * see Bug 40414).<br/>
     *
     * This method makes sure that if the removed sheet was active, another
     * sheet will become active in its place. Furthermore, if the removed sheet
     * was the only selected sheet, another sheet will become selected. The
     * newly active/selected sheet will have the same index, or one less if the
     * removed sheet was the last in the workbook.
     *
    ...
    /**
     * Retrieves the current policy on what to do when getting missing or blank
     * cells from a row. The default is to return blank and null cells.
    ...
    /**
     * Sets the policy on what to do when getting missing or blank cells from a
     * row. This will then apply to all calls to {@link Row#getCell(int)}}. See
     * {@link MissingCellPolicy}
     */
     ...
     *
     * @return integer that contains the index to the active sheet in this book
     * view.
     */
     ...
     *
     * @param index integer that contains the index to the active sheet in this
     * book view.
     */
     ...
     * </pre> To set just repeating rows:
     * <pre>
     *  workbook.setRepeatingRowsAndColumns(0,-1,-1,0,4);
     * </pre> To remove all repeating rows and columns for a sheet.
     * <pre>
     ...
     *
     * @deprecated use {@link XSSFSheet#setRepeatingRows(CellRangeAddress)} or
     * {@link XSSFSheet#setRepeatingColumns(CellRangeAddress)}
     */
     ...
     * styling content, e.g. fonts, cell styles, colors, etc.
     */
    ...
    /**
     * Determines whether a workbook contains the provided sheet name. For the
     * purpose of comparison, long names are truncated to 31 chars.
     *
     * @param name the name to test (case insensitive match)
     * @param excludeSheetIdx the sheet to exclude from the check or -1 to
     * include all sheets in the check.
    ...
    /**
     * Gets a boolean value that indicates whether the date systems used in the
     * workbook starts in 1904.
     * <p>
     * The default value is false, meaning that the workbook uses the 1900 date
     * system, where 1/1/1900 is the first day in the system..
     * </p>
     *
     ...
     * <p>
     * Note that a sheet could instead be set to be very hidden, which is
     * different ({@link #isSheetVeryHidden(int)})
     * </p>
     *
     * @param sheetIx Number
     ...
     * <p>
     * Calling
     * <code>setSheetHidden(sheetIndex, true)</code> is equivalent to
     * <code>setSheetHidden(sheetIndex, Workbook.SHEET_STATE_HIDDEN)</code>.
     * <br/>
     * Calling
     * <code>setSheetHidden(sheetIndex, false)</code> is equivalent to
     ...
     * @throws IllegalArgumentException if the supplied sheet index or state is
     * invalid
     */
    ...
    /**
     * Fired when a formula is deleted from this workbook, for example when
     * calling cell.setCellFormula(null)
     *
     ...
     * <p>
     * The calculation chain object specifies the order in which the cells in a
     * workbook were last calculated
     * </p>
     *
     * @return the <code>CalculationChain</code> object or <code>null</code> if
     * not defined
     */
    @Internal
    ...
    /**
     *
     * @return the helper class used to query the custom XML mapping defined in
     * this workbook
     */
    @Internal
    ...
    /**
     * Specifies a boolean value that indicates whether structure of workbook is
     * locked. <br/>
     * A value true indicates the structure of the workbook is locked.
     * Worksheets in the workbook can't be moved, deleted, hidden, unhidden, or
     * renamed, and new worksheets can't be inserted.<br/>
     ...
     * the same size and position each time the workbook is opened.<br/>
    ...
    /**
     * Specifies a boolean value that indicates whether the workbook is locked
     * for revisions.
     *
     ...
     * <p>
     * The default instance extends the built-in functions with the Excel
     * Analysis Tool Pack. To set / evaluate custom functions you need to
     ...
     * </p>
     *
     * @return wrapped instance of UDFFinder that allows seeking functions both
     * by index and name
     */
    ...
    /**
     * Whether the application shall perform a full recalculation when the
     * workbook is opened.
     * <p>
     * Typically you want to force formula recalculation when you modify cell
     * formulas or values of a workbook previously created by Excel. When set to
     * true, this flag will tell Excel that it needs to recalculate all formulas
     * in the workbook the next time the file is opened.
     * </p>
     * <p>
     * Note, that recalculation updates cached formula results and, thus,
     * modifies the workbook. Depending on the version, Excel may prompt you
     * with "Do you want to save the changes in <em>filename</em>?" on close.
     * </p>
     *
